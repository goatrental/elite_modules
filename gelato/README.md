...
Potřebujete jen Docker. Compose soubor i `config/odoo.conf` jsou **v kořeni
tohoto repozitáře** a sdílí je všechny projekty; složka `gelato/` je jen jedna
z cest v `addons_path`.

```bash
cd modules          # kořen repozitáře, ne tato složka
docker compose up -d
```
